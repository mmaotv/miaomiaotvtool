...
public class DownloadsActivity extends Activity {
    ...
    private void openFile(File file) {
        String lower = file.getName().toLowerCase(Locale.getDefault());
        String ext   = lower.contains(".") ? lower.substring(lower.lastIndexOf('.') + 1) : "";

        // 视频 / 音频 → 直接使用系统应用打开
        if (isVideoExt(ext) || isAudioExt(ext)) {
            openWithSystemIntent(file, ext);
            return;
        }

        // APK → 弹窗选择
        if (ext.equals("apk")) {
            String[] opts = {"📦 内置 APK 安装器", "📱 系统安装器"};
            new AlertDialog.Builder(this)
    ...
    private void openWithSystemIntent(File file, String ext) {
        try {
    ...
    private boolean isAudioExt(String ext) {
        switch (ext) {
            ...
            case "m4a": case "wma": case "ape": case "opus":
                return true;
            default: return false;
        }
    }
